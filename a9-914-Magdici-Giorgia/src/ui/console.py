...
from src.repository.repository import Repository, BookTextFileRepository, ClientTextFileRepository, \
    RentalTextFileRepository, BookBinFileRepository, RentalBinFileRepository, ClientBinFileRepository
from src.services.book_service import BookService, BookException
...
def start():
    ...
    settings_components_list = settings.items()
    settings_dict = {}

    for item in settings_components_list:
        settings_dict[item[0]] = item[1].data
    ...
    if settings_dict["repository"]=="inmemory":
        ...
        book_repo = BookTextFileRepository(book_validator, settings_dict["books"])
        client_repo = ClientTextFileRepository(client_validator, settings_dict["clients"])
        rental_repo = RentalTextFileRepository(rental_validator, settings_dict["rentals"])
    elif settings_dict["repository"]=="binaryfiles":
        book_repo = BookBinFileRepository(book_validator, settings_dict["books"])
        client_repo = ClientBinFileRepository(client_validator, settings_dict["clients"])
        rental_repo = RentalBinFileRepository(rental_validator, settings_dict["rentals"])
    else:
